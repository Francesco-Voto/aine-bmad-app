...
- **Impact:** The UX spec treats optimistic UI as the _defining_ interaction — "Optimistic everywhere — all mutations update the UI before the server responds." The architecture explicitly defers this capability. The implemented product will not behave as the UX specifies until optimistic updates are added.
